...
struct AlarmLiveActivity: Widget {
    var body: some WidgetConfiguration {
        ActivityConfiguration(for: AlarmAttributes<ReminderData>.self) { context in
            ...
            DynamicIsland {
                ...
                DynamicIslandExpandedRegion(.trailing) {
                    reminderCategory(metadata: context.attributes.metadata)
                }
                ...
                AlarmProgressView(reminderCategory: context.attributes.metadata?.category,
    ...
    func lockScreenView(attributes: AlarmAttributes<ReminderData>, state: AlarmPresentationState) -> some View {
        VStack {
            HStack(alignment: .top) {
                alarmTitle(attributes: attributes, state: state)
                Spacer()
                reminderCategory(metadata: attributes.metadata)
            }
    ...
    func bottomView(attributes: AlarmAttributes<ReminderData>, state: AlarmPresentationState) -> some View {
        HStack {
    ...
    @ViewBuilder func alarmTitle(attributes: AlarmAttributes<ReminderData>, state: AlarmPresentationState) -> some View {
    ...
    @ViewBuilder func reminderCategory(metadata: ReminderData?) -> some View {
...
struct AlarmProgressView: View {
    var reminderCategory: ReminderData.Category?
        ...
        Group {
            ...
            case .countdown(let countdown):
                ProgressView(
                    ...
                    label: { EmptyView() },
                    currentValueLabel: {
                        Image(systemName: reminderCategory?.icon ?? "star")
                            .scaleEffect(0.9)
                    })
